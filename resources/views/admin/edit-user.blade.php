      ...
      <form action="/admin/{{ $user->id }}/edit-user" method="post" enctype="multipart/form-data">
        ...
        {{-- <label for="kepala_lab" class="block mb-3">
          ...
          <label for="role" class="block mb-3">
            <span class="block font-semibold mb-1 text-gray-900">Role</span>
            <select name="role" class="px-3 py-2 border shadow rounded-xl w-full block text-sm bg-white border-gray-400 focus:outline-none focus:ring-1 focus:ring-color1 focus:border-color1">
              <option @if($user->role == "0") selected @endif value="0">0 (User)</option>
              <option @if($user->role == "1") selected @endif value="1">1 (Admin)</option>
            </select>
          </label>
